...
impl Pos {
	...
	pub fn to_algebraic(&self) -> String {
		return format!(
			"{}{}",
			('a' as u8 + self.x()) as char,
			('1' as u8 + 7 - self.y()) as char
		);
	}
...
pub fn move_notation(piece: &Piece, target: Pos) -> String {
	if piece.piece_type() == PieceType::Pawn {
		...
		&& (target.x() as i32 - piece.pos().x() as i32).abs() == 2
	{
		// If castle
...
impl Board {
		...
		for (y, rank) in ranks.iter().enumerate() {
			...
			for c in rank.chars() {
				if c.is_numeric() {
					...
					board
						.add(Piece::from_char(c, Pos::new(x as u8, y as u8))?)
						.unwrap();
	...
	pub fn make_move(&mut self, piece: &Piece, target: Pos) -> Result<(), ()> {
		...
		let capture = if let Some(target_piece) = self.piece_at(target) {
			...
			self.remove_at(target_piece.pos())?;
			true
		} else {
			false
		};
		...
		for piece_ref in self.pieces.iter_mut() {
			if *piece_ref == *piece {
				...
				// Check if castled
				if piece.piece_type == PieceType::King
					&& (target.x() as i32 - piece.pos.x() as i32) == 2
				{
		...
		if let Some(piece) = self.piece_at(piece.pos) {
			let moves = match piece.piece_type {
				...
				PieceType::Queen => self
					.generate_straight_moves(piece.pos)
					.into_iter()
					.chain(self.generate_diagonal_moves(piece.pos).into_iter())
					.collect(),
				/*
				PieceType::Queen => {
		...
		if with_castle && !self.is_check(color) {
			if color == Color::Black {
				if self.black_castle_kingside {
					if self.piece_at(Pos::new(5, 0)).is_none()
						&& self.piece_at(Pos::new(6, 0)).is_none()
						&& !self.is_threatened(color, Pos::new(5, 0))
						&& !self.is_threatened(color, Pos::new(6, 0))
					{
				...
				if self.black_castle_queenside {
					if self.piece_at(Pos::new(1, 0)).is_none()
						&& self.piece_at(Pos::new(2, 0)).is_none()
						&& self.piece_at(Pos::new(3, 0)).is_none()
						&& !self.is_threatened(color, Pos::new(1, 0))
						&& !self.is_threatened(color, Pos::new(2, 0))
						&& !self.is_threatened(color, Pos::new(3, 0))
					{
				...
				if self.white_castle_kingside {
					if self.piece_at(Pos::new(5, 7)).is_none()
						&& self.piece_at(Pos::new(6, 7)).is_none()
						&& !self.is_threatened(color, Pos::new(5, 7))
						&& !self.is_threatened(color, Pos::new(6, 7))
					{
				...
				if self.white_castle_queenside {
					if self.piece_at(Pos::new(1, 7)).is_none()
						&& self.piece_at(Pos::new(2, 7)).is_none()
						&& self.piece_at(Pos::new(3, 7)).is_none()
						&& !self.is_threatened(color, Pos::new(1, 7))
						&& !self.is_threatened(color, Pos::new(2, 7))
						&& !self.is_threatened(color, Pos::new(3, 7))
					{
	...
	fn generate_knight_moves(&self, pos: Pos) -> Vec<Pos> {
		...
		for xo in -2..=2 {
			for yo in -2..=2 {
				if (xo as i32).abs().min((yo as i32).abs()) == 1
					&& (xo as i32).abs().max((yo as i32).abs()) == 2
				{
					let x = pos.x() as i32 + xo;
					let y = pos.y() as i32 + yo;
	...
	pub fn is_check(&self, color: Color) -> bool {
		let king_pos = self
			...
			.and_then(|piece| Some(piece.pos()))
			.expect(format!("Could not find {}'s king", color).as_str());
	...
	pub fn is_checkmate(&self, color: Color) -> bool {
		...
		for piece in self.pieces.iter().filter(|piece| piece.color() == color) {
			for piece_move in self.generate_legal_moves(piece).unwrap() {
				...
				board.make_move(&piece, piece_move).ok();
...
impl BoardRenderer {
	pub fn new(aspect: f32) -> BoardRenderer {
		...
		return BoardRenderer {
			...
			texture_mesh: Mesh::with_layout(
				&vec![
					-1.00, -0.75, 0.0, 0.0, 0.0, -0.75, -0.75, 0.0, 1.0, 0.0, -0.75, -1.00, 0.0,
					1.0, 1.0, -1.00, -1.00, 0.0, 0.0, 1.0,
				],
				&vec![0, 1, 2, 0, 2, 3],
				&vec![
	...
	fn render_board(&mut self, view_proj: &Matrix4<f32>, scale: f32) {
		...
		self.board_program
			.set_uniform("white_color", &Vector3::new(0.89, 0.77, 0.69))
			.unwrap();
		self.board_program
			.set_uniform("black_color", &Vector3::new(0.62, 0.43, 0.35))
			.unwrap();

		let mat_model = Matrix4::new_scaling(scale);

		self.board_program
			.set_uniform("mvp", &(view_proj * mat_model))
			.unwrap();
	...
	fn render_square_internal(
		...
		let y = 7 - pos.y();
		let mat_model = Matrix4::new_scaling(scale)
			* Matrix4::new_translation(&Vector3::<f32>::new(x as f32 * 0.25, y as f32 * 0.25, 0.0));
	...
	fn render_piece(&mut self, view_proj: &Matrix4<f32>, scale: f32, piece: &Piece) {
		...
		self.texture_program
			.set_uniform("u_mvp", &(view_proj * mat_model))
			.unwrap();
		render_elements(
			&self.texture_program,
