...
impl Window {
	pub fn new(title: &str, width: u32, height: u32) -> Window {
		...
		let (mut window, events) = glfw
			.create_window(640, 480, "GlVoxel", glfw::WindowMode::Windowed)
...
pub fn render_arrays_instanced(
	shader_program: &ShaderProgram,
	vertex_array: &VertexArray,
	vertex_count: usize,
	instances: usize,
...
pub fn render_elements_instanced(
	shader_program: &ShaderProgram,
	vertex_array: &VertexArray,
	index_buffer: &IndexBuffer,
	instances: usize,
...
impl ShaderProgramBuilder {
	...
	pub fn vertex(&mut self, shader: Shader) -> &mut Self {
		assert!(
			shader.shader_type == ShaderType::Vertex,
			"Type of vertex shader does not match"
		);
	...
	pub fn fragment(&mut self, shader: Shader) -> &mut Self {
		assert!(
			shader.shader_type == ShaderType::Fragment,
			"Type of fragment shader does not match"
		);
	...
	pub fn geometry(&mut self, shader: Shader) -> &mut Self {
		assert!(
			shader.shader_type == ShaderType::Geometry,
			"Type of geometry shader does not match"
		);
	...
	pub fn set_uniform<T: Uniform>(&mut self, name: &str, value: &T) -> Result<(), ShaderError> {
		match self.location_cache.get(name) {
			...
			None => {
				...
				let id = unsafe {
					gl::GetUniformLocation(self.handle, c_name.as_ptr() as *const gl::types::GLchar)
				};
...
lazy_static! {
	static ref SLOT_CACHE: Mutex<Vec<(gl::types::GLuint, u32, gl::types::GLint)>> =
		Mutex::new(Vec::new());
}
...
impl Uniform for Texture {
	unsafe fn set_uniform(&self, id: gl::types::GLint) {
		...
		for i in 0..OPENGL_SLOT_LIMIT {
			...
			let found_slot = slot_cache
				.iter()
				.find(|(handle, slot, bound_id)| *slot == i);
			if found_slot.is_none() {
				Texture::bind(&self, i);

				println!(
					"Texture uniform (handle = {}, slot = {}, id = {})",
					self.handle, i, id
				);
...
impl Mesh {
	pub fn new(vertices: &Vec<f32>, indices: &Vec<u32>) -> Mesh {
		assert!(
			vertices.len() % 3 == 0,
			"Vertices are not properly aligned with layout"
		);
