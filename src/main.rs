...
use rodio::{source::SamplesConverter, source::Source, Decoder, OutputStream, OutputStreamHandle};
use std::fs::File;
...
impl GameInfo {
	...
	fn render(&mut self) {
		...
		self.board_renderer
			.render(&self.board, self.board_scale as f32);

		match self.from_piece {
			Some(piece) => {
				self.board_renderer
					.render_square(self.board_scale as f32, piece.pos(), from_color);

				if let Some(hover) = self.hover {
					self.board_renderer
						.render_square(self.board_scale as f32, hover, to_color);
				}
			},
			None =>
				if let Some(hover) = self.hover {
					self.board_renderer
						.render_square(self.board_scale as f32, hover, from_color);
				},
		}

		if self.board.is_check(Color::White) {
			self.board_renderer.render_square(
				self.board_scale as f32,
				self.board
					.find_pieces(Some(PieceType::King), Some(Color::White), None)
					.first()
					.unwrap()
					.pos(),
				check_color
		...
		if self.board.is_check(Color::Black) {
			self.board_renderer.render_square(
				self.board_scale as f32,
				self.board
					.find_pieces(Some(PieceType::King), Some(Color::Black), None)
					.first()
					.unwrap()
					.pos(),
				check_color
	...
	fn play_move_sound(&self) {
		let source = Decoder::new(BufReader::new(
			File::open("assets/sounds/move.wav").unwrap()
		))
		.unwrap()
		.convert_samples();
...
fn main() {
	...
	let (_stream, stream_handle) =
		OutputStream::try_default().expect("Could not find an audio device");
	...
	let handle = std::thread::spawn(move || {
		for board in from_main {
			...
			let mut moves_list = possible_moves
				.iter()
				.map(|(piece, piece_move)| {
			...
			moves_list.sort_by(|(a, _, _), (b, _, _)| {
			...
			if let Some((eval, piece, target)) = moves_list.last() {
				to_main
					.send((*piece, *target))
					.expect("Failed to return move from thread");
			} else {
				println!("CHECKMATED");
			}
		}
	...
	while !window.should_close() {
		...
		window.swap_buffers();
		glfw.poll_events_unbuffered(
			|window_id: glfw::WindowId, (id, event): (f64, glfw::WindowEvent)| {
...
fn process_event(info: &mut GameInfo, window: &mut glfw::Window, event: &glfw::WindowEvent) {
	match event {
		...
		glfw::WindowEvent::CursorPos(x, y) => {
			...
			let x = if info.window_size.0 > info.window_size.1 {
				x - diff
			} else {
				*x
			};
			let y = if info.window_size.1 > info.window_size.0 {
				y - diff
			} else {
				*y
			};
					...
					match info.from_piece {
						Some(from_piece) => {
							...
							if from_piece.pos() != hover {
								...
								info.board.make_move(&from_piece, hover).expect(
									format!(
										"Failed to make move {}{} (from {})",
										from_piece,
										hover,
										from_piece.pos()
									)
								...
								println!(
									"{} made move {}",
									from_piece.color(),
									move_notation(&from_piece, hover)
								);
								...
								#[cfg(feature = "use_ai")]
								{
									info.worker_thread
										.sender
										.send(info.board.clone())
										.expect("Failed to send board to worker thread");
								}
